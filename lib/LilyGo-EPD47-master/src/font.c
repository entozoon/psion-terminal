...
*/
/*
static void IRAM_ATTR draw_char(sFONT* font, uint8_t *buffer, int *cursor_x, int cursor_y, uint16_t buf_width,
              uint16_t buf_height,FontProperties* props)
{
    uint32_t offset = (ascii_char - '0') * font->Height * (font->Width / 8 + (font->Width % 8 ? 1 : 0));
    uint8_t width = font->Width, height = font->Height;
...
for (int y = 0; y < height; y++) {
    int yy = cursor_y + y;//- glyph->top
    if (yy < 0 || yy >= buf_height) {
        continue;
    }
    int start_pos = *cursor_x +2;//+left
  ...
  //if (font->compressed) {
    //  free(bitmap);
  //}
  *cursor_x += font->Width;//glyph->advance_x;
}
*/
...
void writeln(const GFXfont *font, const char *string, int *cursor_x, int *cursor_y, uint8_t *framebuffer)
{
