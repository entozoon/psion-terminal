...
EpdFontProperties epd_font_properties_default() {
  EpdFontProperties props = {.fg_color = 0,
                             .bg_color = 15,
                             .fallback_glyph = 0,
                             .flags = EPD_DRAW_ALIGN_LEFT};
  return props;
}
...
static int uncompress(uint8_t *dest, uint32_t uncompressed_size,
                      const uint8_t *source, uint32_t source_size) {
  if (uncompressed_size == 0 || dest == NULL || source_size == 0 ||
      source == NULL) {
    return -1;
  }
  tinfl_init(&decomp);
  ...
  tinfl_status decomp_status = tinfl_decompress(
      &decomp, source, &source_size, dest, dest, &uncompressed_size,
...
*/
static enum EpdDrawError IRAM_ATTR draw_char(const EpdFont *font,
                                             uint8_t *buffer, int *cursor_x,
                                             int cursor_y, uint32_t cp,
...
void epd_get_text_bounds(const EpdFont *font, const char *string, const int *x,
                         const int *y, int *x1, int *y1, int *w, int *h,
  ...
  while ((c = next_cp((const uint8_t **)&string))) {
    get_char_bounds(font, c, &temp_x, &temp_y, &minx, &miny, &maxx, &maxy,
                    &props);
  }
...
static enum EpdDrawError epd_write_line(const EpdFont *font, const char *string,
...
enum EpdDrawError epd_write_default(const EpdFont *font, const char *string,
                                    int *cursor_x, int *cursor_y,
                                    uint8_t *framebuffer) {
  const EpdFontProperties props = epd_font_properties_default();
  return epd_write_string(font, string, cursor_x, cursor_y, framebuffer,
                          &props);
}
...
enum EpdDrawError epd_write_string(const EpdFont *font, const char *string,
  ...
  while ((token = strsep(&newstring, "\n")) != NULL) {
    *cursor_x = line_start;
    err |= epd_write_line(font, token, cursor_x, cursor_y, framebuffer,
                          properties);
